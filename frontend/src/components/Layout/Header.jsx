        ...
        <>
            <div className={`${styles.section}`}>
                <div className="hidden 800px:h-[50px] 800px:my-[20px] 800px:flex items-center justify-between ">
                    <div>
                        <Link to="/">
                            <img src="https://shopo.quomodothemes.website/assets/images/logo.svg" alt="" width={150} height={150} />
                        </Link>
                    </div>
                        ...
                        <AiOutlineSearch size={30} className='absolute right-2 top-1.5 cursor-pointer' />
                        {
                            searchTerm && searchData && searchData.length !== 0 ? (
                                <div className="absolute min-h-[30vh] bg-slate-50 shadow-sm-2 z-[9] p-4">
                                    {
                                        searchData && searchData.map((val, idx) => {
                                            const d = val.name;
                                            const productName = d.replace(/\s+/g, "-")
                                            return (
                                                <Link to={`/product/${productName}`} key={val}>
                                                    <div className="w-full flex items-start py-3">
                                                        <img src={val.image_Url[0].url} alt={productName} className='w-[40px] h-[40px] mr-[10px]' />
                                                        <h1>{val.name}</h1>
                                                    </div>
                                                </Link>
                                            )
            ...
            <div className={`${active ? "shadow-sm fixed top-0 left-0 z-10" : null} transition hidden 800px:flex items-center justify-between w-full bg-[#3321cB] h-[70px]`}>
